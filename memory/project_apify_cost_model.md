...
**Our per-result rates are approximate and plan-blind.** Published HarvestAPI (per 1K): Profile Details (Full) $4/1K = $0.004; Full+email $10/1K = $0.01; Company-employees basic $3/1K Free/Starter vs $1.50/1K Business (plan-dependent 2×). Our flat rates don't scale by tier. (Update: the LinkedIn panel's `DEPTHS` estimate constants are now 0.004 / 0.008 / 0.014 for short / full / full+email; the older modal `full`=0.01 / 0.012 constants no longer exist.) The code comment already flags "Short-mode search pricing confirmed on first live run."
...
**vdrmota waterfall was FULLY REMOVED (2026-08-24; this superseded the earlier "retire as default" plan).** Measured: its lead-scraped event is per-LEAD (~$0.10 free tier / ~$0.005 paid) and our input pulls 10 leads/company ⇒ ~$1.00/company free tier; the pricing.ts estimate treated it as $0.005/contact (~100× off). Its one real run ($3.96, aborted at our 60s budget) filled ZERO contact fields, and its `leadsEnrichment` is a people-DATABASE join (Mongo ObjectIds, licdn photo URLs), not reproducible by scraping, but replaceable for our need by pattern-permutation + Million Verifier (~$0.004/contact). Replacement plan (settings panel, pattern+MV, own HTTPS→Playwright scraper, size routing): [[RESUME-WATERFALL-SETTINGS]] at repo root.

**2026-08-30 update (spend audit). Canonical source is now [`docs/APIFY_ACTOR_COSTS.md`](../docs/APIFY_ACTOR_COSTS.md); trust a fresh `node scripts/pull-actor-costs.mjs` over any number in this memory.**
- **`lead-scraped` (the compass business-leads add-on) was missing from the compass event list above.** It bills **$0.0075 per PERSON found** (BRONZE; pay-on-hit, zero-lead places cost $0), bounded by `maximumLeadsEnrichmentRecords`, whose cap is **PER PLACE, not per run** ("Maximum leads per place"): always send ≤3. `leadsEnrichmentDepartments` can filter server-side.
- **The $14.17 incident (2026-08-30):** a probe sent `maximumLeadsEnrichmentRecords: 400` believing it a per-run cap; chains returned whole rosters (1,836 `lead-scraped` × $0.0075 = $14.17 on a $5 authorization). At cap 3 the same run is ~$1.02. This is the single most dangerous compass cost; never enable the add-on without pinning the per-place cap. Produced [`docs/APIFY_ACTOR_COSTS.md`](../docs/APIFY_ACTOR_COSTS.md) + its pre-run protocol.
- **site_scrape band top:** the ~$0.0005-0.003/site figure above is the cheap-tier typical; browser-tier escalation can reach **~$0.01/site**.
