    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl p-6 text-white" style={{ background: 'linear-gradient(135deg, #4f46e5, #7c3aed, #6366f1)', boxShadow: '0 10px 30px -5px rgba(99, 102, 241, 0.2)' }}>
        <div className="relative z-10">
          <p className="text-sm font-medium text-white/70">Client Submissions</p>
          <h1 className="text-2xl font-bold mt-1">Lead Feedback</h1>
          <p className="text-sm text-white/60 mt-1">
            {total} total submissions from clients
          </p>
        </div>
                ...
                {feedback.map((f) => (
                  <TableRow key={f.id}>
                    <TableCell>
                      ...
                      {f.lead_company && (
                        <p className="text-xs text-muted-foreground">{f.lead_company}</p>
                      )}
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {campaignMap.get(f.campaign_id) || "—"}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="secondary"
