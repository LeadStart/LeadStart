...
export default function ClientDashboardPage() {
  ...
  const fetchSnapshots = useCallback(async () => {
    ...
    setSnapshots((data || []) as CampaignSnapshot[]);
    setLoading(false);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            ...
            {campaigns.length === 0 ? (
              ...
              <div className="space-y-2">
                {campaigns.map((campaign) => {
                  ...
                  return (
                    <Link key={campaign.id} href={`/client/campaigns/${campaign.id}`} className="group flex items-center justify-between rounded-xl border border-border/50 p-4 transition-all duration-200 hover:border-indigo-200 hover:shadow-md hover:bg-indigo-50/30">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground">{campMetrics.emails_sent.toLocaleString()} sent &middot; {campMetrics.meetings_booked} positive</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge variant="secondary" className={campaign.status === "active" ? "bg-emerald-100 text-emerald-800 border border-emerald-200" : "bg-gray-100 text-gray-600 border border-gray-200"}>
                          {campaign.status}
                        </Badge>
        ...
        <Card className="border-border/50 shadow-sm">
            ...
            <CardContent>
              {reports.length === 0 ? (
                <p className="text-sm text-muted-foreground">No reports yet. KPI reports will appear here.</p>
              ) : (
