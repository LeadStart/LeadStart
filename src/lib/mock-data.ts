import type {
  Client,
  Campaign,
  CampaignSnapshot,
  LeadFeedback,
  KPIReport,
  WebhookEvent,
  Profile,
  Organization,
  Contact,
  PricingPlan,
  Quote,
  ClientSubscription,
  BillingInvoice,
  PaymentLink,
} from "@/types/app";

// ---------- Organization ----------
export const MOCK_ORG: Organization = {
  id: "00000000-0000-0000-0000-000000000001",
  name: "LeadStart Agency",
  instantly_api_key: "demo_key_placeholder",
  instantly_workspace_id: "demo_workspace",
  created_at: "2025-01-01T00:00:00Z",
  updated_at: "2025-01-01T00:00:00Z",
};

// ---------- Profiles ----------
export const MOCK_PROFILES: Profile[] = [
  {
    id: "user-owner-001",
    email: "admin@leadstart.com",
    full_name: "Daniel (Owner)",
    role: "owner",
    organization_id: MOCK_ORG.id,
    is_active: true,
    created_at: "2025-01-01T00:00:00Z",
    updated_at: "2025-01-01T00:00:00Z",
  },
  {
    id: "user-va-001",
    email: "va@leadstart.com",
    full_name: "Sarah (VA)",
    role: "va",
    organization_id: MOCK_ORG.id,
    is_active: true,
    created_at: "2025-02-01T00:00:00Z",
    updated_at: "2025-02-01T00:00:00Z",
  },
  {
    id: "user-client-001",
    email: "john@acmecorp.com",
    full_name: "John (Acme)",
    role: "client",
    organization_id: MOCK_ORG.id,
    is_active: true,
    created_at: "2025-01-15T00:00:00Z",
    updated_at: "2025-01-15T00:00:00Z",
  },
  {
    id: "user-client-001b",
    email: "ops@acmecorp.com",
    full_name: "Priya (Acme Ops)",
    role: "client",
    organization_id: MOCK_ORG.id,
    is_active: true,
    created_at: "2025-02-20T00:00:00Z",
    updated_at: "2025-02-20T00:00:00Z",
  },
  {
    id: "user-client-002",
    email: "lisa@techstartup.io",
    full_name: "Lisa (TechStartup)",
    role: "client",
    organization_id: MOCK_ORG.id,
    is_active: true,
    created_at: "2025-02-01T00:00:00Z",
    updated_at: "2025-02-01T00:00:00Z",
  },
];

// ---------- Clients ----------
export const MOCK_CLIENTS: Client[] = [
  {
    id: "client-001",
    organization_id: MOCK_ORG.id,
    name: "Acme Corp",
    contact_email: "john@acmecorp.com",
    notes: "Real estate investor — focus on commercial leads",
    report_interval_days: 7,
    report_schedule_start: "2025-03-03",
    report_last_sent_at: "2025-03-17T15:00:00Z",
    report_recipients: ["john@acmecorp.com"],
    stripe_customer_id: "cus_demo_acme",
    created_at: "2025-01-15T00:00:00Z",
    updated_at: "2025-03-01T00:00:00Z",
  },
  {
    id: "client-002",
    organization_id: MOCK_ORG.id,
    name: "TechStartup Inc",
    contact_email: "lisa@techstartup.io",
    notes: "SaaS founder — targeting CTOs at mid-market companies",
    report_interval_days: null,
    report_schedule_start: null,
    report_last_sent_at: null,
    report_recipients: null,
    stripe_customer_id: "cus_demo_techstartup",
    created_at: "2025-02-01T00:00:00Z",
    updated_at: "2025-03-10T00:00:00Z",
  },
  {
    id: "client-003",
    organization_id: MOCK_ORG.id,
    name: "GrowthCo Marketing",
    contact_email: "mike@growthco.com",
    notes: "New client — onboarding in progress",
    report_interval_days: null,
    report_schedule_start: null,
    report_last_sent_at: null,
    report_recipients: null,
    stripe_customer_id: "cus_demo_growthco",
    created_at: "2025-03-15T00:00:00Z",
    updated_at: "2025-03-15T00:00:00Z",
  },
];

// ---------- Client Users (join table) ----------
export const MOCK_CLIENT_USERS = [
  { client_id: "client-001", user_id: "user-client-001", created_at: "2025-01-15T00:00:00Z", invite_status: "active" },
  { client_id: "client-001", user_id: "user-client-001b", created_at: "2025-02-20T00:00:00Z", invite_status: "active" },
  { client_id: "client-002", user_id: "user-client-002", created_at: "2025-02-01T00:00:00Z", invite_status: "active" },
];

// ---------- Campaigns ----------
export const MOCK_CAMPAIGNS: Campaign[] = [
  {
    id: "camp-001",
    client_id: "client-001",
    organization_id: MOCK_ORG.id,
    instantly_campaign_id: "inst_camp_abc123",
    name: "Acme — Commercial RE Investors Q1",
    status: "active",
    created_at: "2025-01-20T00:00:00Z",
    updated_at: "2025-03-01T00:00:00Z",
  },
  {
    id: "camp-002",
    client_id: "client-001",
    organization_id: MOCK_ORG.id,
    instantly_campaign_id: "inst_camp_def456",
    name: "Acme — Multifamily Owners",
    status: "active",
    created_at: "2025-02-10T00:00:00Z",
    updated_at: "2025-03-10T00:00:00Z",
  },
  {
    id: "camp-003",
    client_id: "client-002",
    organization_id: MOCK_ORG.id,
    instantly_campaign_id: "inst_camp_ghi789",
    name: "TechStartup — CTO Outreach",
    status: "active",
    created_at: "2025-02-15T00:00:00Z",
    updated_at: "2025-03-15T00:00:00Z",
  },
  {
    id: "camp-004",
    client_id: "client-002",
    organization_id: MOCK_ORG.id,
    instantly_campaign_id: "inst_camp_jkl012",
    name: "TechStartup — VP Eng Warm-up",
    status: "paused",
    created_at: "2025-03-01T00:00:00Z",
    updated_at: "2025-03-12T00:00:00Z",
  },
];

// ---------- Snapshots (last 30 days of daily data) ----------
function generateSnapshots(): CampaignSnapshot[] {
  const snapshots: CampaignSnapshot[] = [];
  const activeCampaigns = ["camp-001", "camp-002", "camp-003"];

  for (const campId of activeCampaigns) {
    for (let dayOffset = 29; dayOffset >= 0; dayOffset--) {
      const date = new Date();
      date.setDate(date.getDate() - dayOffset);
      const dateStr = date.toISOString().split("T")[0];

      // Vary data per campaign
      const baseMultiplier = campId === "camp-001" ? 1.2 : campId === "camp-002" ? 0.8 : 1.0;
      const dailyVariation = 0.7 + Math.random() * 0.6; // 0.7 to 1.3

      const sent = Math.round(45 * baseMultiplier * dailyVariation);
      const replies = Math.round(sent * (0.04 + Math.random() * 0.04)); // 4-8% reply
      const uniqueReplies = Math.max(1, replies - Math.floor(Math.random() * 2));
      const positiveReplies = Math.round(uniqueReplies * (0.3 + Math.random() * 0.3));
      const bounces = Math.round(sent * (0.01 + Math.random() * 0.02));
      const unsubscribes = Math.random() > 0.8 ? 1 : 0;
      const meetings = Math.random() > 0.7 ? (Math.random() > 0.5 ? 2 : 1) : 0;

      snapshots.push({
        id: `snap-${campId}-${dateStr}`,
        campaign_id: campId,
        snapshot_date: dateStr,
        total_leads: Math.round(500 * baseMultiplier),
        emails_sent: sent,
        replies,
        unique_replies: uniqueReplies,
        positive_replies: positiveReplies,
        bounces,
        unsubscribes,
        meetings_booked: meetings,
        new_leads_contacted: Math.round(sent * 0.6),
        reply_rate: sent > 0 ? Number(((uniqueReplies / sent) * 100).toFixed(2)) : 0,
        positive_reply_rate: uniqueReplies > 0 ? Number(((positiveReplies / uniqueReplies) * 100).toFixed(2)) : 0,
        bounce_rate: sent > 0 ? Number(((bounces / sent) * 100).toFixed(2)) : 0,
        unsubscribe_rate: sent > 0 ? Number(((unsubscribes / sent) * 100).toFixed(2)) : 0,
        raw_data: null,
        fetched_at: new Date().toISOString(),
      });
    }
  }

  return snapshots;
}

export const MOCK_SNAPSHOTS: CampaignSnapshot[] = generateSnapshots();

// ---------- Feedback ----------
export const MOCK_FEEDBACK: LeadFeedback[] = [
  {
    id: "fb-001",
    campaign_id: "camp-001",
    lead_email: "mark.johnson@bigcorp.com",
    lead_name: "Mark Johnson",
    lead_company: "BigCorp Holdings",
    status: "good_lead",
    comment: "Responded positively, wants to schedule a call next week",
    submitted_by: "user-client-001",
    created_at: "2025-03-20T14:30:00Z",
  },
  {
    id: "fb-002",
    campaign_id: "camp-001",
    lead_email: "sarah.williams@realtyfund.com",
    lead_name: "Sarah Williams",
    lead_company: "Realty Fund LLC",
    status: "interested",
    comment: "Asked for more information about our services",
    submitted_by: "user-client-001",
    created_at: "2025-03-19T10:00:00Z",
  },
  {
    id: "fb-003",
    campaign_id: "camp-001",
    lead_email: "tom.davis@nocorp.com",
    lead_name: "Tom Davis",
    lead_company: "NoCorp Inc",
    status: "bad_lead",
    comment: "Wrong industry — they do residential only",
    submitted_by: "user-client-001",
    created_at: "2025-03-18T16:45:00Z",
  },
  {
    id: "fb-004",
    campaign_id: "camp-003",
    lead_email: "jennifer.lee@saasco.io",
    lead_name: "Jennifer Lee",
    lead_company: "SaaSCo",
    status: "good_lead",
    comment: "Great fit — already using competitor, wants demo",
    submitted_by: "user-client-002",
    created_at: "2025-03-21T09:15:00Z",
  },
  {
    id: "fb-005",
    campaign_id: "camp-003",
    lead_email: "alex.chen@devtools.com",
    lead_name: "Alex Chen",
    lead_company: "DevTools Inc",
    status: "already_contacted",
    comment: "We already spoke to them last month",
    submitted_by: "user-client-002",
    created_at: "2025-03-17T11:30:00Z",
  },
  {
    id: "fb-006",
    campaign_id: "camp-002",
    lead_email: "robert.kim@apartments.net",
    lead_name: "Robert Kim",
    lead_company: "Metro Apartments",
    status: "wrong_person",
    comment: "Not the decision maker — referred us to their VP",
    submitted_by: "user-client-001",
    created_at: "2025-03-16T13:00:00Z",
  },
  {
    id: "fb-007",
    campaign_id: "camp-003",
    lead_email: "diana.patel@cloudops.io",
    lead_name: "Diana Patel",
    lead_company: "CloudOps",
    status: "not_interested",
    comment: "Said they just signed a 2-year contract with someone else",
    submitted_by: "user-client-002",
    created_at: "2025-03-15T08:45:00Z",
  },
];

// ---------- Webhook Events ----------
export const MOCK_EVENTS: WebhookEvent[] = [
  {
    id: "evt-001",
    organization_id: MOCK_ORG.id,
    event_type: "email_replied",
    campaign_instantly_id: "inst_camp_abc123",
    lead_email: "mark.johnson@bigcorp.com",
    payload: { reply_type: "positive" },
    processed: true,
    excluded: false,
    received_at: "2025-03-23T10:30:00Z",
  },
  {
    id: "evt-002",
    organization_id: MOCK_ORG.id,
    event_type: "email_sent",
    campaign_instantly_id: "inst_camp_abc123",
    lead_email: "new.lead@prospect.com",
    payload: {},
    processed: true,
    excluded: false,
    received_at: "2025-03-23T09:15:00Z",
  },
  {
    id: "evt-003",
    organization_id: MOCK_ORG.id,
    event_type: "email_bounced",
    campaign_instantly_id: "inst_camp_ghi789",
    lead_email: "invalid@old-domain.com",
    payload: { bounce_type: "hard" },
    processed: true,
    excluded: false,
    received_at: "2025-03-22T22:00:00Z",
  },
  {
    id: "evt-004",
    organization_id: MOCK_ORG.id,
    event_type: "meeting_booked",
    campaign_instantly_id: "inst_camp_abc123",
    lead_email: "sarah.williams@realtyfund.com",
    payload: { meeting_date: "2025-03-28" },
    processed: true,
    excluded: false,
    received_at: "2025-03-22T15:45:00Z",
  },
  {
    id: "evt-005",
    organization_id: MOCK_ORG.id,
    event_type: "email_replied",
    campaign_instantly_id: "inst_camp_ghi789",
    lead_email: "jennifer.lee@saasco.io",
    payload: { reply_type: "positive" },
    processed: true,
    excluded: false,
    received_at: "2025-03-22T11:30:00Z",
  },
];

// ---------- KPI Reports ----------
export const MOCK_REPORTS: KPIReport[] = [
  {
    id: "report-001",
    client_id: "client-001",
    organization_id: MOCK_ORG.id,
    report_period_start: "2025-03-01",
    report_period_end: "2025-03-15",
    report_data: {
      client_name: "Acme Corp",
      period: { start: "2025-03-01", end: "2025-03-15" },
      campaigns: [
        {
          campaign_name: "Acme — Commercial RE Investors Q1",
          campaign_id: "camp-001",
          metrics: {
            emails_sent: 680,
            replies: 41,
            unique_replies: 38,
            positive_replies: 15,
            bounces: 12,
            unsubscribes: 2,
            meetings_booked: 6,
            reply_rate: 5.59,
            positive_reply_rate: 39.47,
            bounce_rate: 1.76,
            unsubscribe_rate: 0.29,
            reply_to_meeting_rate: 15.79,
          },
        },
      ],
      totals: {
        emails_sent: 680,
        replies: 41,
        unique_replies: 38,
        positive_replies: 15,
        bounces: 12,
        unsubscribes: 2,
        meetings_booked: 6,
        reply_rate: 5.59,
        positive_reply_rate: 39.47,
        bounce_rate: 1.76,
        unsubscribe_rate: 0.29,
        reply_to_meeting_rate: 15.79,
      },
    },
    sent_at: "2025-03-16T09:00:00Z",
    sent_to: ["john@acmecorp.com"],
    created_by: "user-owner-001",
    created_at: "2025-03-16T09:00:00Z",
  },
  {
    id: "report-002",
    client_id: "client-002",
    organization_id: MOCK_ORG.id,
    report_period_start: "2025-03-01",
    report_period_end: "2025-03-15",
    report_data: {
      client_name: "TechStartup Inc",
      period: { start: "2025-03-01", end: "2025-03-15" },
      campaigns: [
        {
          campaign_name: "TechStartup — CTO Outreach",
          campaign_id: "camp-003",
          metrics: {
            emails_sent: 540,
            replies: 35,
            unique_replies: 30,
            positive_replies: 12,
            bounces: 8,
            unsubscribes: 1,
            meetings_booked: 4,
            reply_rate: 5.56,
            positive_reply_rate: 40.0,
            bounce_rate: 1.48,
            unsubscribe_rate: 0.19,
            reply_to_meeting_rate: 13.33,
          },
        },
      ],
      totals: {
        emails_sent: 540,
        replies: 35,
        unique_replies: 30,
        positive_replies: 12,
        bounces: 8,
        unsubscribes: 1,
        meetings_booked: 4,
        reply_rate: 5.56,
        positive_reply_rate: 40.0,
        bounce_rate: 1.48,
        unsubscribe_rate: 0.19,
        reply_to_meeting_rate: 13.33,
      },
    },
    sent_at: null,
    sent_to: null,
    created_by: "user-owner-001",
    created_at: "2025-03-16T09:30:00Z",
  },
];

// ---------- Billing ----------
export const MOCK_PRICING_PLANS: PricingPlan[] = [
  {
    id: "plan-starter",
    organization_id: MOCK_ORG.id,
    slug: "starter",
    name: "Starter",
    description: "For founders validating cold outbound",
    features: ["1 campaign", "Up to 500 leads/mo", "Weekly KPI reports", "Basic support"],
    monthly_price_cents: 49700,
    currency: "usd",
    stripe_product_id: "prod_demo_starter",
    stripe_monthly_price_id: "price_demo_starter_monthly",
    scope_template: "Inbox setup + 14-day warming\n1 cold outbound campaign, up to 500 leads/mo\nWeekly KPI report",
    active: true,
    sort_order: 1,
    created_at: "2025-11-01T00:00:00Z",
    updated_at: "2025-11-01T00:00:00Z",
  },
  {
    id: "plan-growth",
    organization_id: MOCK_ORG.id,
    slug: "growth",
    name: "Growth",
    description: "Our most popular plan for scaling teams",
    features: [
      "3 campaigns",
      "Up to 2,000 leads/mo",
      "Daily KPI reports",
      "Priority support",
      "Client portal access",
    ],
    monthly_price_cents: 99700,
    currency: "usd",
    stripe_product_id: "prod_demo_growth",
    stripe_monthly_price_id: "price_demo_growth_monthly",
    scope_template: "Inbox setup + 14-day warming\nUp to 3 concurrent campaigns, 2,000 leads/mo\nDaily KPI reports + client portal access\nPriority support",
    active: true,
    sort_order: 2,
    created_at: "2025-11-01T00:00:00Z",
    updated_at: "2025-11-01T00:00:00Z",
  },
  {
    id: "plan-scale",
    organization_id: MOCK_ORG.id,
    slug: "scale",
    name: "Scale",
    description: "White-glove outbound for larger agencies",
    features: [
      "Unlimited campaigns",
      "Unlimited leads",
      "Real-time reporting",
      "Dedicated account manager",
      "Custom integrations",
      "White-label portal",
    ],
    monthly_price_cents: 199700,
    currency: "usd",
    stripe_product_id: "prod_demo_scale",
    stripe_monthly_price_id: "price_demo_scale_monthly",
    scope_template: "Inbox setup + 14-day warming\nUnlimited campaigns and leads\nReal-time reporting, dedicated AM\nWhite-label client portal + custom integrations",
    active: true,
    sort_order: 3,
    created_at: "2025-11-01T00:00:00Z",
    updated_at: "2025-11-01T00:00:00Z",
  },
];

export const MOCK_QUOTES: Quote[] = [
  {
    id: "quote-001",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    quote_number: "Q-2025-0001",
    plan_id: "plan-growth",
    plan_name_snapshot: "Growth",
    monthly_price_cents: 99700,
    setup_fee_cents: 150000,
    currency: "usd",
    scope_of_work:
      "Inbox setup + 14-day warming\nUp to 3 concurrent campaigns\nWeekly KPI reports",
    terms: "Auto-charged monthly via Stripe after the 14-day warming period. Net 0.",
    signed_url_hash: "demo_hash_001",
    status: "accepted",
    expires_at: "2025-12-22T00:00:00Z",
    sent_at: "2025-12-10T15:00:00Z",
    viewed_at: "2025-12-10T18:30:00Z",
    accepted_at: "2025-12-15T09:15:00Z",
    declined_at: null,
    sent_to_email: "john@acmecorp.com",
    sent_by: "user-owner-001",
    accepted_by_email: "john@acmecorp.com",
    accepted_ip: "203.0.113.42",
    accepted_user_agent: "Mozilla/5.0 (Macintosh; Intel Mac OS X 14_1) Safari/605.1.15",
    stripe_checkout_session_id: "cs_demo_acme_001",
    created_at: "2025-12-10T14:00:00Z",
    updated_at: "2025-12-15T09:15:00Z",
  },
  {
    id: "quote-002",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    quote_number: "Q-2026-0001",
    plan_id: "plan-starter",
    plan_name_snapshot: "Starter",
    monthly_price_cents: 49700,
    setup_fee_cents: 75000,
    currency: "usd",
    scope_of_work: "Inbox setup + 14-day warming\n1 campaign, up to 500 leads/mo\nWeekly KPI report",
    terms: "Auto-charged monthly via Stripe after warming period.",
    signed_url_hash: "demo_hash_002",
    status: "accepted",
    expires_at: "2026-01-15T00:00:00Z",
    sent_at: "2026-01-05T11:00:00Z",
    viewed_at: "2026-01-05T11:42:00Z",
    accepted_at: "2026-01-07T16:20:00Z",
    declined_at: null,
    sent_to_email: "lisa@techstartup.io",
    sent_by: "user-owner-001",
    accepted_by_email: "lisa@techstartup.io",
    accepted_ip: "198.51.100.23",
    accepted_user_agent: "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) Safari/604.1",
    stripe_checkout_session_id: "cs_demo_techstartup_001",
    created_at: "2026-01-05T10:30:00Z",
    updated_at: "2026-01-07T16:20:00Z",
  },
  {
    id: "quote-003",
    organization_id: MOCK_ORG.id,
    client_id: "client-003",
    quote_number: "Q-2026-0002",
    plan_id: "plan-scale",
    plan_name_snapshot: "Scale",
    monthly_price_cents: 199700,
    setup_fee_cents: 300000,
    currency: "usd",
    scope_of_work:
      "Inbox setup + 14-day warming\nUnlimited campaigns\nDedicated account manager\nWhite-label portal",
    terms: "Net 0. Auto-charged monthly via Stripe after 14-day warming period.",
    signed_url_hash: "demo_hash_003",
    status: "accepted",
    expires_at: "2026-04-20T00:00:00Z",
    sent_at: "2026-04-08T14:00:00Z",
    viewed_at: "2026-04-08T16:12:00Z",
    accepted_at: "2026-04-10T10:05:00Z",
    declined_at: null,
    sent_to_email: "mike@growthco.com",
    sent_by: "user-owner-001",
    accepted_by_email: "mike@growthco.com",
    accepted_ip: "192.0.2.88",
    accepted_user_agent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/121.0",
    stripe_checkout_session_id: "cs_demo_growthco_001",
    created_at: "2026-04-08T13:30:00Z",
    updated_at: "2026-04-10T10:05:00Z",
  },
  {
    id: "quote-004",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    quote_number: "Q-2026-0003",
    plan_id: "plan-scale",
    plan_name_snapshot: "Scale",
    monthly_price_cents: 199700,
    setup_fee_cents: 0,
    currency: "usd",
    scope_of_work:
      "Upgrade to Scale — unlimited campaigns, dedicated AM, white-label portal.\n(No new setup fee — prorating against existing Growth subscription.)",
    terms: "Replaces active Growth subscription. Upgrade effective at next billing cycle.",
    signed_url_hash: "demo_hash_004",
    status: "sent",
    expires_at: "2026-04-24T00:00:00Z",
    sent_at: "2026-04-17T09:00:00Z",
    viewed_at: "2026-04-17T13:45:00Z",
    accepted_at: null,
    declined_at: null,
    sent_to_email: "john@acmecorp.com",
    sent_by: "user-owner-001",
    accepted_by_email: null,
    accepted_ip: null,
    accepted_user_agent: null,
    stripe_checkout_session_id: null,
    created_at: "2026-04-17T08:30:00Z",
    updated_at: "2026-04-17T13:45:00Z",
  },
  {
    id: "quote-005",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    quote_number: "Q-2026-0004",
    plan_id: "plan-growth",
    plan_name_snapshot: "Growth",
    monthly_price_cents: 99700,
    setup_fee_cents: 50000,
    currency: "usd",
    scope_of_work: "Upgrade from Starter to Growth (+2 campaigns, daily reports).",
    terms: "Upgrade effective at next billing cycle.",
    signed_url_hash: "demo_hash_005",
    status: "draft",
    expires_at: null,
    sent_at: null,
    viewed_at: null,
    accepted_at: null,
    declined_at: null,
    sent_to_email: null,
    sent_by: null,
    accepted_by_email: null,
    accepted_ip: null,
    accepted_user_agent: null,
    stripe_checkout_session_id: null,
    created_at: "2026-04-18T16:00:00Z",
    updated_at: "2026-04-18T16:00:00Z",
  },
];

export const MOCK_CLIENT_SUBSCRIPTIONS: ClientSubscription[] = [
  {
    id: "sub-001",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    plan_id: "plan-growth",
    quote_id: "quote-001",
    stripe_customer_id: "cus_demo_acme",
    stripe_subscription_id: "sub_demo_acme",
    status: "active",
    trial_end: "2025-12-29T00:00:00Z",
    current_period_start: "2026-03-29T00:00:00Z",
    current_period_end: "2026-04-29T00:00:00Z",
    cancel_at_period_end: false,
    canceled_at: null,
    setup_fee_cents: 150000,
    setup_fee_paid_at: "2025-12-15T09:15:00Z",
    warming_days_at_signup: 14,
    created_at: "2025-12-15T09:15:00Z",
    updated_at: "2026-03-29T00:00:00Z",
  },
  {
    id: "sub-002",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    plan_id: "plan-starter",
    quote_id: "quote-002",
    stripe_customer_id: "cus_demo_techstartup",
    stripe_subscription_id: "sub_demo_techstartup",
    status: "active",
    trial_end: "2026-01-21T00:00:00Z",
    current_period_start: "2026-03-21T00:00:00Z",
    current_period_end: "2026-04-21T00:00:00Z",
    cancel_at_period_end: false,
    canceled_at: null,
    setup_fee_cents: 75000,
    setup_fee_paid_at: "2026-01-07T16:20:00Z",
    warming_days_at_signup: 14,
    created_at: "2026-01-07T16:20:00Z",
    updated_at: "2026-03-21T00:00:00Z",
  },
  {
    id: "sub-003",
    organization_id: MOCK_ORG.id,
    client_id: "client-003",
    plan_id: "plan-scale",
    quote_id: "quote-003",
    stripe_customer_id: "cus_demo_growthco",
    stripe_subscription_id: "sub_demo_growthco",
    status: "trialing",
    trial_end: "2026-04-24T10:05:00Z",
    current_period_start: "2026-04-10T10:05:00Z",
    current_period_end: "2026-04-24T10:05:00Z",
    cancel_at_period_end: false,
    canceled_at: null,
    setup_fee_cents: 300000,
    setup_fee_paid_at: "2026-04-10T10:05:00Z",
    warming_days_at_signup: 14,
    created_at: "2026-04-10T10:05:00Z",
    updated_at: "2026-04-10T10:05:00Z",
  },
];

export const MOCK_BILLING_INVOICES: BillingInvoice[] = [
  // Acme — Growth, 3 months of monthly invoices
  {
    id: "in_demo_acme_04",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    stripe_customer_id: "cus_demo_acme",
    stripe_subscription_id: "sub_demo_acme",
    stripe_invoice_number: "INV-0012",
    amount_cents: 99700,
    amount_paid_cents: 99700,
    amount_due_cents: 0,
    currency: "usd",
    status: "paid",
    period_start: "2026-03-29T00:00:00Z",
    period_end: "2026-04-29T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/acme/04",
    invoice_pdf_url: "https://invoice.stripe.com/demo/acme/04.pdf",
    issued_at: "2026-03-29T00:00:00Z",
    paid_at: "2026-03-29T00:05:00Z",
    created_at: "2026-03-29T00:00:00Z",
    updated_at: "2026-03-29T00:05:00Z",
  },
  {
    id: "in_demo_acme_03",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    stripe_customer_id: "cus_demo_acme",
    stripe_subscription_id: "sub_demo_acme",
    stripe_invoice_number: "INV-0009",
    amount_cents: 99700,
    amount_paid_cents: 99700,
    amount_due_cents: 0,
    currency: "usd",
    status: "paid",
    period_start: "2026-02-27T00:00:00Z",
    period_end: "2026-03-29T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/acme/03",
    invoice_pdf_url: "https://invoice.stripe.com/demo/acme/03.pdf",
    issued_at: "2026-02-27T00:00:00Z",
    paid_at: "2026-02-27T00:04:00Z",
    created_at: "2026-02-27T00:00:00Z",
    updated_at: "2026-02-27T00:04:00Z",
  },
  {
    id: "in_demo_acme_02",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    stripe_customer_id: "cus_demo_acme",
    stripe_subscription_id: "sub_demo_acme",
    stripe_invoice_number: "INV-0006",
    amount_cents: 99700,
    amount_paid_cents: 99700,
    amount_due_cents: 0,
    currency: "usd",
    status: "paid",
    period_start: "2026-01-28T00:00:00Z",
    period_end: "2026-02-27T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/acme/02",
    invoice_pdf_url: "https://invoice.stripe.com/demo/acme/02.pdf",
    issued_at: "2026-01-28T00:00:00Z",
    paid_at: "2026-01-28T00:03:00Z",
    created_at: "2026-01-28T00:00:00Z",
    updated_at: "2026-01-28T00:03:00Z",
  },
  // TechStartup — Starter
  {
    id: "in_demo_tech_03",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    stripe_customer_id: "cus_demo_techstartup",
    stripe_subscription_id: "sub_demo_techstartup",
    stripe_invoice_number: "INV-0011",
    amount_cents: 49700,
    amount_paid_cents: 0,
    amount_due_cents: 49700,
    currency: "usd",
    status: "open",
    period_start: "2026-03-21T00:00:00Z",
    period_end: "2026-04-21T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/tech/03",
    invoice_pdf_url: "https://invoice.stripe.com/demo/tech/03.pdf",
    issued_at: "2026-03-21T00:00:00Z",
    paid_at: null,
    created_at: "2026-03-21T00:00:00Z",
    updated_at: "2026-03-21T00:00:00Z",
  },
  {
    id: "in_demo_tech_02",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    stripe_customer_id: "cus_demo_techstartup",
    stripe_subscription_id: "sub_demo_techstartup",
    stripe_invoice_number: "INV-0008",
    amount_cents: 49700,
    amount_paid_cents: 49700,
    amount_due_cents: 0,
    currency: "usd",
    status: "paid",
    period_start: "2026-02-19T00:00:00Z",
    period_end: "2026-03-21T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/tech/02",
    invoice_pdf_url: "https://invoice.stripe.com/demo/tech/02.pdf",
    issued_at: "2026-02-19T00:00:00Z",
    paid_at: "2026-02-19T00:05:00Z",
    created_at: "2026-02-19T00:00:00Z",
    updated_at: "2026-02-19T00:05:00Z",
  },
  {
    id: "in_demo_tech_01",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    stripe_customer_id: "cus_demo_techstartup",
    stripe_subscription_id: "sub_demo_techstartup",
    stripe_invoice_number: "INV-0005",
    amount_cents: 49700 + 75000,
    amount_paid_cents: 49700 + 75000,
    amount_due_cents: 0,
    currency: "usd",
    status: "paid",
    period_start: "2026-01-21T00:00:00Z",
    period_end: "2026-02-19T00:00:00Z",
    hosted_invoice_url: "https://invoice.stripe.com/demo/tech/01",
    invoice_pdf_url: "https://invoice.stripe.com/demo/tech/01.pdf",
    issued_at: "2026-01-21T00:00:00Z",
    paid_at: "2026-01-21T00:02:00Z",
    created_at: "2026-01-07T16:20:00Z",
    updated_at: "2026-01-21T00:02:00Z",
  },
];

export const MOCK_PAYMENT_LINKS: PaymentLink[] = [
  {
    id: "pl-001",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    quote_id: "quote-001",
    stripe_checkout_session_id: "cs_demo_acme_001",
    stripe_checkout_url: "https://checkout.stripe.com/demo/cs_demo_acme_001",
    status: "completed",
    created_at: "2025-12-15T09:10:00Z",
    expires_at: "2025-12-16T09:10:00Z",
    completed_at: "2025-12-15T09:15:00Z",
  },
  {
    id: "pl-002",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    quote_id: "quote-002",
    stripe_checkout_session_id: "cs_demo_techstartup_001",
    stripe_checkout_url: "https://checkout.stripe.com/demo/cs_demo_techstartup_001",
    status: "completed",
    created_at: "2026-01-07T16:15:00Z",
    expires_at: "2026-01-08T16:15:00Z",
    completed_at: "2026-01-07T16:20:00Z",
  },
  {
    id: "pl-003",
    organization_id: MOCK_ORG.id,
    client_id: "client-003",
    quote_id: "quote-003",
    stripe_checkout_session_id: "cs_demo_growthco_001",
    stripe_checkout_url: "https://checkout.stripe.com/demo/cs_demo_growthco_001",
    status: "completed",
    created_at: "2026-04-10T10:00:00Z",
    expires_at: "2026-04-11T10:00:00Z",
    completed_at: "2026-04-10T10:05:00Z",
  },
];

// ---------- Step Metrics (for step-level health analysis) ----------
import type { CampaignStepMetric } from "@/types/app";

function generateStepMetrics(): CampaignStepMetric[] {
  const metrics: CampaignStepMetric[] = [];
  const campaigns = ["camp-001", "camp-002", "camp-003"];

  for (const campId of campaigns) {
    for (let step = 1; step <= 3; step++) {
      // Generate 4 weekly periods
      for (let weekOffset = 3; weekOffset >= 0; weekOffset--) {
        const end = new Date();
        end.setDate(end.getDate() - weekOffset * 7);
        const start = new Date(end);
        start.setDate(start.getDate() - 7);

        const baseSent = Math.round(40 + Math.random() * 30);
        // Step 1 has higher reply rate, drops for later steps
        const baseReplyRate = step === 1 ? 8 + Math.random() * 4 : step === 2 ? 4 + Math.random() * 3 : 2 + Math.random() * 2;

        // For camp-003, simulate a drop in step 1 reply rate in the latest period
        let replyRate = baseReplyRate;
        if (campId === "camp-003" && step === 1 && weekOffset === 0) {
          replyRate = 2.5; // Dropped from ~10% to 2.5%
        }

        const replies = Math.round(baseSent * (replyRate / 100));
        const opens = Math.round(baseSent * (0.3 + Math.random() * 0.2));
        const bounces = Math.round(baseSent * (0.01 + Math.random() * 0.02));

        metrics.push({
          id: `step-${campId}-s${step}-w${weekOffset}`,
          campaign_id: campId,
          step,
          period_start: start.toISOString().split("T")[0],
          period_end: end.toISOString().split("T")[0],
          sent: baseSent,
          replies,
          unique_replies: replies,
          opens,
          unique_opens: opens,
          bounces,
          reply_rate: Number(replyRate.toFixed(2)),
          open_rate: baseSent > 0 ? Number(((opens / baseSent) * 100).toFixed(2)) : 0,
          bounce_rate: baseSent > 0 ? Number(((bounces / baseSent) * 100).toFixed(2)) : 0,
          fetched_at: new Date().toISOString(),
        });
      }
    }
  }

  return metrics;
}

export const MOCK_STEP_METRICS: CampaignStepMetric[] = generateStepMetrics();

// ---------- Contacts ----------
// Contacts are the single source of truth for a person. When pipeline_stage
// is non-null, the contact is "in the pipeline". No separate prospects table.
export const MOCK_CONTACTS: Contact[] = [
  {
    id: "contact-001",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "James",
    last_name: "Walker",
    email: "james@blueskyroofing.com",
    company_name: "BlueSky Roofing",
    title: "Owner",
    phone: "(555) 234-5678",
    linkedin_url: "https://linkedin.com/in/jwalker",
    intro_line: "Noticed BlueSky just expanded into commercial roofing.",
    enrichment_data: { industry: "Home Services", website: "blueskyroofing.com" },
    tags: ["home-services", "decision-maker"],
    status: "enriched",
    source: "referral",
    notes: null,
    pipeline_stage: "meeting",
    pipeline_sort_order: 0,
    pipeline_notes: "Wants to target commercial property managers. Meeting Thursday 3pm.",
    pipeline_follow_up_date: "2025-03-27",
    pipeline_added_at: "2025-03-10T00:00:00Z",
    created_at: "2025-03-08T00:00:00Z",
    updated_at: "2025-03-22T00:00:00Z",
  },
  {
    id: "contact-002",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Rachel",
    last_name: "Kim",
    email: "rachel@apexfa.com",
    company_name: "Apex Financial Advisors",
    title: "Managing Partner",
    phone: "(555) 876-5432",
    linkedin_url: "https://linkedin.com/in/rachelkim",
    intro_line: "Saw Apex was named a top RIA by Forbes.",
    enrichment_data: { industry: "Financial Services", website: "apexfa.com" },
    tags: ["financial", "HNW"],
    status: "enriched",
    source: "linkedin",
    notes: null,
    pipeline_stage: "proposal",
    pipeline_sort_order: 0,
    pipeline_notes: "Sent Growth plan proposal ($997/mo). Waiting on decision. Targeting HNW individuals.",
    pipeline_follow_up_date: "2025-03-25",
    pipeline_added_at: "2025-03-05T00:00:00Z",
    created_at: "2025-03-03T00:00:00Z",
    updated_at: "2025-03-20T00:00:00Z",
  },
  {
    id: "contact-003",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "David",
    last_name: "Chen",
    email: "david@novatech.io",
    company_name: "NovaTech Solutions",
    title: "CTO",
    phone: null,
    linkedin_url: "https://linkedin.com/in/davidchen",
    intro_line: "Saw NovaTech just raised Series B.",
    enrichment_data: { industry: "SaaS / Tech", website: "novatech.io" },
    tags: ["saas", "cto"],
    status: "enriched",
    source: "linkedin",
    notes: null,
    pipeline_stage: "contacted",
    pipeline_sort_order: 0,
    pipeline_notes: "Reached out via LinkedIn. Interested in CTO outreach campaign.",
    pipeline_follow_up_date: "2025-03-26",
    pipeline_added_at: "2025-03-18T00:00:00Z",
    created_at: "2025-03-16T00:00:00Z",
    updated_at: "2025-03-18T00:00:00Z",
  },
  {
    id: "contact-004",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Patricia",
    last_name: "Morales",
    email: "pmorales@summitlegal.com",
    company_name: "Summit Legal Group",
    title: "Founding Partner",
    phone: "(555) 321-9876",
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "Legal", website: "summitlegal.com" },
    tags: ["legal", "referral"],
    status: "new",
    source: "referral",
    notes: null,
    pipeline_stage: "lead",
    pipeline_sort_order: 0,
    pipeline_notes: "Referred by Acme Corp. Hasn't been contacted yet.",
    pipeline_follow_up_date: null,
    pipeline_added_at: "2025-03-21T00:00:00Z",
    created_at: "2025-03-20T00:00:00Z",
    updated_at: "2025-03-21T00:00:00Z",
  },
  {
    id: "contact-005",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Marcus",
    last_name: "Thompson",
    email: "marcus@greenfieldprop.com",
    company_name: "Greenfield Properties",
    title: "Director of Marketing",
    phone: "(555) 654-3210",
    linkedin_url: "https://linkedin.com/in/marcust",
    intro_line: "Noticed Greenfield just launched a new condo portfolio.",
    enrichment_data: { industry: "Real Estate", website: "greenfieldprop.com" },
    tags: ["real-estate"],
    status: "active",
    source: "inbound",
    notes: null,
    pipeline_stage: "closed",
    pipeline_sort_order: 0,
    pipeline_notes: "Signed on Scale plan. Converted to client — see GrowthCo Marketing.",
    pipeline_follow_up_date: null,
    pipeline_added_at: "2025-02-15T00:00:00Z",
    created_at: "2025-02-14T00:00:00Z",
    updated_at: "2025-03-15T00:00:00Z",
  },
  {
    id: "contact-006",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Amy",
    last_name: "Liu",
    email: "amy@fasttracklog.com",
    company_name: "FastTrack Logistics",
    title: "VP Operations",
    phone: null,
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "Logistics", website: "fasttracklog.com" },
    tags: ["logistics"],
    status: "bounced",
    source: "cold-outreach",
    notes: null,
    pipeline_stage: "lost",
    pipeline_sort_order: 0,
    pipeline_notes: "Went with a competitor. Budget was the main concern. Re-engage in Q3.",
    pipeline_follow_up_date: "2025-07-01",
    pipeline_added_at: "2025-02-01T00:00:00Z",
    created_at: "2025-01-30T00:00:00Z",
    updated_at: "2025-03-05T00:00:00Z",
  },
  // Contacts NOT yet in the pipeline — appear in the "Add to pipeline" picker
  {
    id: "contact-007",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Erica",
    last_name: "Patel",
    email: "erica@northwindhealth.com",
    company_name: "Northwind Health",
    title: "Head of Growth",
    phone: null,
    linkedin_url: "https://linkedin.com/in/ericapatel",
    intro_line: null,
    enrichment_data: { industry: "Healthtech", website: "northwindhealth.com" },
    tags: ["healthtech"],
    status: "enriched",
    source: "linkedin",
    notes: null,
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-02T00:00:00Z",
    updated_at: "2025-04-02T00:00:00Z",
  },
  {
    id: "contact-008",
    organization_id: MOCK_ORG.id,
    client_id: null,
    campaign_id: null,
    first_name: "Tom",
    last_name: "Reed",
    email: "tom@redwoodautomation.com",
    company_name: "Redwood Automation",
    title: "Founder",
    phone: "(555) 111-2020",
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "Industrial / B2B", website: "redwoodautomation.com" },
    tags: ["b2b"],
    status: "new",
    source: "list-import",
    notes: null,
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-05T00:00:00Z",
    updated_at: "2025-04-05T00:00:00Z",
  },
  // Client-owned contacts — recipients on our clients' cold email campaigns.
  // These never appear in the Prospects kanban (that's LeadStart's funnel).
  {
    id: "contact-101",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    campaign_id: null,
    first_name: "Dana",
    last_name: "Ortiz",
    email: "dana.ortiz@pinecrestdev.com",
    company_name: "Pinecrest Development",
    title: "Director of Acquisitions",
    phone: "(555) 777-3100",
    linkedin_url: "https://linkedin.com/in/danaortiz",
    intro_line: null,
    enrichment_data: { industry: "Commercial Real Estate", website: "pinecrestdev.com" },
    tags: ["commercial-re", "acquisitions"],
    status: "active",
    source: "apollo",
    notes: "On Acme Corp's commercial RE campaign.",
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-10T00:00:00Z",
    updated_at: "2025-04-10T00:00:00Z",
  },
  {
    id: "contact-102",
    organization_id: MOCK_ORG.id,
    client_id: "client-001",
    campaign_id: null,
    first_name: "Brian",
    last_name: "Falk",
    email: "brian@harborstonepm.com",
    company_name: "Harborstone Property Mgmt",
    title: "Managing Director",
    phone: null,
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "Property Management", website: "harborstonepm.com" },
    tags: ["property-mgmt"],
    status: "replied",
    source: "list-import",
    notes: null,
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-09T00:00:00Z",
    updated_at: "2025-04-11T00:00:00Z",
  },
  {
    id: "contact-103",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    campaign_id: null,
    first_name: "Priya",
    last_name: "Ganesh",
    email: "priya.g@lumenlabs.ai",
    company_name: "Lumen Labs",
    title: "CTO",
    phone: null,
    linkedin_url: "https://linkedin.com/in/priyaganesh",
    intro_line: null,
    enrichment_data: { industry: "SaaS / Tech", website: "lumenlabs.ai" },
    tags: ["cto", "saas"],
    status: "enriched",
    source: "apollo",
    notes: "On TechStartup Inc's CTO outreach list.",
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-08T00:00:00Z",
    updated_at: "2025-04-08T00:00:00Z",
  },
  {
    id: "contact-104",
    organization_id: MOCK_ORG.id,
    client_id: "client-002",
    campaign_id: null,
    first_name: "Max",
    last_name: "Huang",
    email: "max@vectorcloud.io",
    company_name: "VectorCloud",
    title: "VP Engineering",
    phone: null,
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "SaaS / Tech", website: "vectorcloud.io" },
    tags: ["vp-eng", "saas"],
    status: "bounced",
    source: "apollo",
    notes: null,
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-07T00:00:00Z",
    updated_at: "2025-04-12T00:00:00Z",
  },
  {
    id: "contact-105",
    organization_id: MOCK_ORG.id,
    client_id: "client-003",
    campaign_id: null,
    first_name: "Sofia",
    last_name: "Arellano",
    email: "sofia@luminaestates.com",
    company_name: "Lumina Estates",
    title: "Principal Broker",
    phone: "(555) 212-0099",
    linkedin_url: null,
    intro_line: null,
    enrichment_data: { industry: "Real Estate", website: "luminaestates.com" },
    tags: ["broker", "luxury-re"],
    status: "new",
    source: "list-import",
    notes: null,
    pipeline_stage: null,
    pipeline_sort_order: 0,
    pipeline_notes: null,
    pipeline_follow_up_date: null,
    pipeline_added_at: null,
    created_at: "2025-04-06T00:00:00Z",
    updated_at: "2025-04-06T00:00:00Z",
  },
];
