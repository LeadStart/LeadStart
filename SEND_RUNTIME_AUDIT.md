...
| SEND-54 | medium (spend) | **RESOLVED 2026-09-06** (was CONFIRMED, PARKED) | run-apify-enrichment/route.ts:2139-2157; apify/pattern-mv.ts:137-139 | The pattern finder's paid MV verdict is never written to the contact cache, so the send gate re-verifies and re-bills the same address at first send | Code reproduction by Finder D; the verify PHASE does cache (1695-1700) | FIXED: `pattern_mv` now carries its raw MV response out (`PatternMvOutcome.mvResponse`) and `writeEmail` stamps the verdict onto the contact's 30-day cache (`decideFromResult` patch) in the same fill-only write, so the verify phase and the send gate reuse it (no re-verify, no re-bill) |
...
## Addendum 2026-09-25: SEND-08 never took effect (reopened and fixed)

SEND-08 was verified by code reproduction only. Its fix read `Action:`/`Status:` from the `message/delivery-status` part's own body, but the Gmail API never returns that part with a body: it splits it into child parts and returns the fields as the children's headers and/or body text. Replaying the parser over the 26 real notices in the four sending mailboxes found `Action`/`Status` null on every one, so classification kept falling back to text guessing. Consequences: final Gmail "(Failure)" notices quoting a 4.x.x code were filed SOFT (one unreachable address received all 6 steps, 18 notices, Jul 11 to Sep 13); Microsoft 365 / Mimecast reports on their own thread lost their recipient and were dropped (one contact still active and mid-sequence); no bounce recorded its code/class, so a Gmail "very low reputation of the sending domain" rejection (5.7.350, Sep 4) was stored as an ordinary bounce. Recorded hard bounces: 3; actual: 14. Fixed by `classifyBounce` / `readDsn` (mime.ts), `findBouncedSend` (src/lib/native/bounce-attribution.ts), migration 00131 (bounce_code / bounce_class / bounce_diagnostic), a `mail_rejected_as_spam` owner alert, real-shape fixtures in scripts/test-bounce-parsing.ts, and scripts/backfill-native-bounces.ts for history. Lesson: parser fixes are verified against real payloads replayed from the mailboxes, never against the DB alone (the DB only holds what the parser already decided).
