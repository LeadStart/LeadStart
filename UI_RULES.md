...
| **Sidebar** | **Dark solid panel** (`--sidebar` slate-900), light text, solid brand active pill. The old white→indigo gradient is gone. |
...
- Decorative gradients anywhere (backgrounds, buttons, badges, sidebar, hero, table headers, login). Functional gradients (CSS mask tricks) are the only exemption.
...
- **Sidebar** (`layout/sidebar.tsx`): solid `bg-sidebar` dark panel, `border-r border-sidebar-border`, light text; active item = solid `bg-sidebar-primary text-sidebar-primary-foreground rounded-lg`, hover = `bg-white/10`. No notch, no cast-shadow div, no gradient.
